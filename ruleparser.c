    ...
    while (ts_next(&ts, &t) && t.kind) {
        if (t.kind == TOK_IDENTIFIER) {
            ...
            if (*op1.start == '@'){
                ...
                is_dec = slice_lit_match(token_to_slice(tag), "param", false) || slice_lit_match(token_to_slice(tag), "func", false) || slice_lit_match(token_to_slice(tag), "dec", false) || slice_lit_match(token_to_slice(tag), "label", false) || slice_lit_match(token_to_slice(tag), "struct", false);
                if (!ts_next(&ts, &op1)){
                    print("Unfinished rule %v",token_to_slice(t));
                    return -1;
                }
