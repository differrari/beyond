...
bool find_symbol(sem_rule type, Token t){
...
bool analyze_rule(int current_rule, int curr_option){
    ...
    if (sym.sym_type == sem_rule_func || sym.sym_type == sem_rule_param || sym.sym_type == sem_rule_label || sym.sym_type == sem_rule_dec || sym.sym_type == sem_rule_func){
        symbol_table[symbol_count++] = sym;
        print("SYMBOL = %s TYPE = %v NAME = %v",sem_rule_to_string(sym.sym_type),sym.type.kind ? token_to_slice(sym.type) : make_string_slice("none", 0, 4),token_to_slice(sym.name));
    }
