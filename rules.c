#include "rules.h"
typedef enum { rule_block,rule_statement,rule_separator,rule_assignment,rule_funccall,rule_funcdec,rule_funcarguments,rule_argdec,rule_argument,rule_argname,rule_conditional,rule_condition,rule_declaration,rule_jump,rule_label,rule_expression,rule_chain,rule_math,rule_variable,rule_forloop,rule_whileloop,rule_dowhile, num_grammar_rules } grammar_rules;
...
grammar_rule language_rules[num_grammar_rules] = {
	...
	[rule_funcdec] = {{
		{{ SYMDEC(IDENTIFIER,type), SYMDEC(IDENTIFIER,name), SYMRULE(funcarguments,param), TOKEN(LBRACE), SYMRULE(block,scope), TOKEN(RBRACE),  },6},{{ SYMDEC(IDENTIFIER,name), SYMRULE(funcarguments,param), TOKEN(LBRACE), SYMRULE(block,scope), TOKEN(RBRACE),  },5},{{ SYMDEC(IDENTIFIER,type), SYMDEC(IDENTIFIER,name), TOKEN(LBRACE), SYMRULE(block,scope), TOKEN(RBRACE),  },5},{{ SYMDEC(IDENTIFIER,name), TOKEN(LBRACE), SYMRULE(block,scope), TOKEN(RBRACE),  },4},{{ SYMDEC(IDENTIFIER,type), SYMDEC(IDENTIFIER,name), SYMRULE(funcarguments,param), TOKEN(LBRACE), TOKEN(RBRACE),  },5},{{ SYMDEC(IDENTIFIER,name), SYMRULE(funcarguments,param), TOKEN(LBRACE), TOKEN(RBRACE),  },4},{{ SYMDEC(IDENTIFIER,type), SYMDEC(IDENTIFIER,name), TOKEN(LBRACE), TOKEN(RBRACE),  },4},{{ SYMDEC(IDENTIFIER,name), TOKEN(LBRACE), TOKEN(RBRACE),  },3},
	},8, sem_func},
	[rule_funcarguments] = {{
